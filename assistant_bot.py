...
TOOL_SCHEMAS = [
    {
        ...
        "function": {
            "name": "generate_image",
            "description": "Сгенерировать ОДНО изображение по описанию. Используй, только если просят нарисовать/создать картинку. Аргумент prompt — запрос пользователя (можно на русском или английском); инструмент САМ переведёт его в английский SDXL-промпт. Не вызывай повторно.",
            "parameters": {
                "type": "object",
                "properties": {"prompt": {"type": "string", "description": "детальное описание изображения на английском, с явным названием объекта"}},
                "required": ["prompt"],
            },
        },
    },
